...
function aiq($queryID, $results, $description="send Poll", $spt="Vai al bot"){
	global $api;
	$url="https://api.telegram.org/bot$api/answerInlineQuery?inline_query_id=$queryID";
...
function iEdit($inline_message_id, $msg, $menu, $type='inline_keyboard', $parse_mode="html")
{
    ...
    $msg = urlencode($msg);
	$query="https://api.telegram.org/bot$api/editMessageText?inline_message_id=$inline_message_id&text=$msg&parse_mode=$parse_mode";
...
if(strpos($msg,"/update")===0){
    ...
    if($iMessageID){ iedit($iMessageID, $r, $menu); if($update){ edit("", "", $menu, "", $msgID, $cbid, "Statistiche Aggiornate!", true); } else { edit("", "", $menu, "", $msgID, $cbid, "Voto Aggiunto!", true); } exit;}
    if($cbdata){ edit($chatID, $r, $menu, "", $msgID, $cbid, "", true);  if($update){ edit("", "", $menu, "", $msgID, $cbid, "Statistiche Aggiornate!", true); } else { edit("", "", $menu, "", $msgID, $cbid, "Voto Aggiunto!", true); } exit; } 
}
